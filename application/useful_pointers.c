...
int main(void) {
    ...
    // arrays from a function. The reason for this is when a program is compiled, C must know 
    // how large a return type needs to be. An array, unlike an int, has an unknown size to the 
    // compiler. Pointers on the other hand are known to have a set size. This means that we can 
    // create the array, then return a pointer to it.

    // Be careful, it takes a bit more effort than that though.
    ...
    // telling the computer to free the held memory
    free(name_2);
}
